...
{
    /// <summary>
    /// Port of Controllers/UsersController.cs. Original routing:
    ///   [Route("api/users")]
    ///   [Authorize] (class-level - protects every action)
    ///   [HttpGet]           GetAll()  -> GET   api/users
    ///   [HttpGet("{id}")]   GetById() -> GET   api/users/{id}
    ///   [HttpPatch("{id}")] Patch()   -> PATCH api/users/{id}
    ///
    /// None of these carry [AllowAnonymous]; JwtAuthMiddleware is default-deny, so none of the
    /// ported functions below carry Flowboard.Functions.Middleware.[AllowAnonymous] either -
    /// they stay protected.
    ///
    /// Authenticated user access: JwtAuthMiddleware validates the bearer token and assigns the
    /// resulting ClaimsPrincipal directly to httpContext.User (see
    /// Flowboard.Functions/Middleware/JwtAuthMiddleware.cs), so req.HttpContext.User is the
    /// same ClaimsPrincipal the original controller read via the ControllerBase.User property.
    ///
    /// Forbid() mapping: the original's `return Forbid("...")` resolves to
    /// ControllerBase.Forbid(params string[] authenticationSchemes) - the string is treated as
    /// an authentication scheme name (not a response body), and ForbidResult delegates to
    /// IAuthenticationService.ForbidAsync for that scheme. This app registers no
    /// IAuthenticationService at all (auth is done entirely by JwtAuthMiddleware), so calling
    /// Forbid() here - or, for that matter, hitting this code path in the original app if it
    /// also lacks a matching scheme handler - throws at request time rather than returning a
    /// clean 403. It's mapped to an equivalent empty 403 (`StatusCodeResult(403)`), which is
    /// the intended status code and the best-effort faithful behavior without reproducing a
    /// crash.
    /// </summary>
    public class UsersFunctions
    {
        ...
        public UsersFunctions(MongoDbService mongoDbService)
        {
        ...
        private static object UserToResponse(FlowModels.User user)
        {
            return new
            {
                id = user.Id,
            ...
            string id)
        {
            ...
            return new OkObjectResult(UserToResponse(user));
        }

        // PATCH /api/users/{id} - Partial update (only provided fields are updated)
        [Function("Users_Patch")]
        public async Task<IActionResult> Patch(
            [HttpTrigger(AuthorizationLevel.Anonymous, "patch", Route = "api/users/{id}")] HttpRequest req,
            string id)
        {
            ...
            try
            {
                updates = await req.ReadFromJsonAsync<Dictionary<string, object>>();
            }
            catch
            {
                updates = null;
            ...
            if (existingUser == null) return new NotFoundObjectResult(new { message = "User not found." });

            var requesterId = GetUserIdFromToken(req);
            if (requesterId == null || (requesterId != id && !req.HttpContext.User.IsInRole("Admin")))
            ...
            if (updates.TryGetValue("email", out var emailObj) && emailObj != null)
            {
                ...
                if (!string.IsNullOrEmpty(emailStr))
                {
                    ...
                    if (existing != null && existing.Id != id)
                        return new ObjectResult(new { message = "Email already in use by another user." })
                        {
                    ...
                    case "user_img_base64":
                        ...
                        else if (!string.IsNullOrWhiteSpace(stringValue))
                        {
                            ...
                            if (bytes != null)
                            {
                                ...
                                {
                                    return new BadRequestObjectResult(new { message = "Image size exceeds 5MB limit." });
                                }
            ...
            try
            {
                ...
                if (updatedUser == null) return new NotFoundObjectResult(new { message = "User not found after update." });

                return new OkObjectResult(UserToResponse(updatedUser));
            }
            catch (Exception ex)
            {
                return new ObjectResult(new { message = "Failed to update user.", detail = ex.Message })
                {
